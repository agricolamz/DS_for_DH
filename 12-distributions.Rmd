...
library(checkdown)
autocheck_question(question_id = 12.1, 
                   answer = "нет", 
                   options = c("да", "нет"),
                   right = "правильно", 
...
```{r, echo = FALSE, results='asis'}
library(checkdown)
autocheck_question(question_id = 12.2, 
                   answer = "color", 
...
```{r, echo = FALSE, results='asis'}
autocheck_question(question_id = 12.3, 
                   answer = 0.103, 
                   right = "правильно", 
                   wrong = "у меня другой ответ")
```
...
```{block, type = "rmdtask"}
Рассмотрим задачу, которую обсуждали выше с именами героев из датасета `strawars`. Посчитайте значение функции плотности в точке 10 для биномиального распределения. (округление до 2 знаков после запятой).
```

```{r, echo = FALSE, results='asis'}
autocheck_question(question_id = 12.4, 
                   answer = round(dbinom(x = 10, 87+5, prob = 0.103), 3), 
                   right = "правильно", 
                   wrong = "у меня другой ответ")
```
...
## Првоерка нулевой гипотезы

Теперь мы обсудим стандартный трюк, который получил большую популярность в XX веке, и сейчас повсеместно используется. Этот трюк, к сожалению, помогает лишь показать, что что-то отличается, так что мы немножко переиначим наши задачи.

Представим себе, что я исследую героев Звездных войн. Я верю, что герои с именем на "B" встречаются в Звездных войнах с вероятностью 0.103. В новом фильме из 13 новых персонажей 4 имеют имя на "B". Является ли эта разница статистически значимой?

Создадим две гипотезы:

* $H_0$ --- (нулевая гипотеза) наблюдаемый результаты происходят из того же распределения что и другие.
* $H_1$ --- (альтернативная гипотеза) наблюдаемый результаты происходят из разных распределений.

Давайте посчитаем вероятность наблюдать такой или более экстремальный результат, если верна нулевая гипотеза (p-value, p-уровень значимости):
...
Это можно сделать при помощи следующей функции:
