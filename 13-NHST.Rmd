...
## Obamacare
```{block, type = "rmdtask"}
В 2010 Б. Обама подписал закон о доступном здравоохранении. В [датасет](https://raw.githubusercontent.com/agricolamz/DS_for_DH/master/data/obamacare.csv) записаны данные о доле незастрахованных людей (в процентах) в каждом штате в 2010 и в 2015 годах. Проведите статистический тест, показывающий что произошло изменение.
```

```{r, echo=FALSE, message=FALSE}
obamacare <- read_csv("https://raw.githubusercontent.com/agricolamz/DS_for_DH/master/data/obamacare.csv")

obamacare.test <- t.test(obamacare$`Uninsured Rate (2010)`, obamacare$`Uninsured Rate (2015)`, data = obamacare, paired = TRUE)
```
