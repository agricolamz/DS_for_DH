...
* Не каждый год содержит 365 дней. Существуют высокосные года.
...
Как видно, из этих функций в R можно работать как с датами, так и с временем. В качестве иллюстрации мы будем использовать датасет `flights` из пакета `nycflights13`, в котором содержатся данные по полетам из Нью Йорка в 2013 года.
