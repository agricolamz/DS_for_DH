...
```{r}
df.short <- tibble(consonant = c("stops", "fricatives", "affricates", "nasals"),
...
```{r, echo = FALSE}
df.short %>% 
  pivot_longer(names_to = "position", values_to = "number", initial:final)->
  df.long
df.long
```
...
```{r}
df.short %>% 
  pivot_longer(names_to = "position", values_to = "number", -consonant) ->
  df.long
