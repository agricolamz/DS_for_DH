...
```{r,  echo=FALSE, message=FALSE}
merged %>% 
  select(language_s2, census_1885, census_1895, census_1926, census_2010) %>% 
  pivot_longer(names_to = "census", values_to = "n_people", census_1885:census_2010) %>% 
  group_by(language_s2, census) %>% 
  summarise(n = sum(n_people)) %>% 
  pivot_wider(names_from = "census", values_from = "n")
```
